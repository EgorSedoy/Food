...
function calc() {
  //Calculator    
  var result = document.querySelector('.calculating__result span'); //получаем элемент для результата

  var sex, height, weight, age, ratio;

  if (localStorage.getItem('sex')) {
    //если есть значение в локальном хранилище
...
function cards() {
  ...
  (0,_services_services_js__WEBPACK_IMPORTED_MODULE_3__.getResource)('http://localhost:3000/menu').then(function (data) {
...
function form(formSelector, modalTimerId) {
   ...
   */
  var forms = document.querySelectorAll(formSelector); //получаем все формы
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (event) {
      ...
      var statusMessage = document.createElement('img'); //создаём элемент для вывода сообщения
      ...
      var json = JSON.stringify(Object.fromEntries(formData.entries())); //преобразуем данные в json

      (0,_services_services__WEBPACK_IMPORTED_MODULE_8__.postData)('http://localhost:3000/requests', json) //передаём данные на сервер
      .then(function (data) {
        console.log(data);
...
function openModal(modalSelector, modalTimerId) {
  var modal = document.querySelector(modalSelector); //получаем элемент модального окна
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTrigger.forEach(function (item) {
    //привязываем обработчик клика ко всем элементам модального окна
  ...
  modal.addEventListener('click', function (e) {
    ...
    if (e.target === modal || e.target.getAttribute('data-close') == '') {
      //если клик произошел не внутри элемента модального окна
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.code === 'Escape' && modal.classList.contains('show')) {
      //если произошло нажатие на клавишу Esc
      closeModal(modalSelector);
    }
  });
...
function slider(_ref) {
  ...
  var indicators = document.createElement('ol'),
      //создаём элемент для индикаторов
  ...
  for (var i = 0; i < slides.length; i++) {
    //перебираем все слайды
    var dot = document.createElement('li'); //создаём элемент для индикатора
  ...
  function deleteNotDigits(str) {
    //удаляем все не цифры из строки
    return +str.replace(/\D/g, '');
  }

  next.addEventListener('click', function () {
    //обрабатываем клик по кнопке "следующий"
    ...
    slidesField.style.transform = "translateX(-".concat(offset, "px)"); //перемещаем обертку влево
  ...
  dots.forEach(function (dot) {
    ...
    dot.addEventListener('click', function (event) {
      //обрабатываем клик по индикатору
      var slideTo = event.target.getAttribute('data-slide-to'); //получаем номер слайда
...
function timer(id, deadline) {
  function getTimeRamaining(endtime) {
    ...
    var t = Date.parse(endtime) - Date.parse(new Date()); //получаем время до конца события
  ...
  function getZero(num) {
    //если число меньше 10, добавляем ноль
    if (num >= 0 && num < 10) {
      return "0".concat(num);
    } else {
      return num;
  ...
  function setClock(selector, endtime) {
    //получаем объект с оставшимися днями, часами, минутами и секундами
    var timer = document.querySelector(selector),
        //получаем элемент для отображения оставшегося времени
    days = timer.querySelector('#days'),
        //получаем элемент для дней
    hours = timer.querySelector('#hours'),
        //получаем элемент для часов
    minutes = timer.querySelector('#minutes'),
        //получаем элемент для минут
    seconds = timer.querySelector('#seconds'),
        //получаем элемент для секунд
    ...
    function updateClock() {
      ...
      var t = getTimeRamaining(endtime); //получаем объект с временем
...
(__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill)(); // promise for IE
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2023-11-01');
